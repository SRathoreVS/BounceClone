class Level {
  constructor(game, n) {
    ...
    this.game.checkpoint = {
      x: this.map.player.x * Tile.size,
      y: this.map.player.y * Tile.size,
    };
    if (this.map.enemies) {
  ...
  inBounds(i, j) {
    if (i < 0 || i >= this.map.tiles[0].length) return false;

    if (j < 0 || j >= this.map.tiles.length) return false;
  ...
  clearTile(x, y, z) {
    ...
    if (this.inBounds(i, j)) {
      ...
      this.map.tiles[j] =
        line.slice(0, +(i - 1) + 1 || undefined) + k + line.slice(i + 1);
    }
  }
  ...
  showGbar() {
    ...
    for (
      let j = 0;
      j < this.map.rings - Math.ceil(this.ringsCollected / 2);
      j++
    ) {
      ringX = 80 + j * 15;
      ...
      for (let i = 0; i < line.length; i++) {
        ...
        if (tile === "G") {
          if (Math.ceil(this.ringsCollected / 2) === this.map.rings) {
            ...
            currentLine =
              currentLine.slice(0, +(i - 1) + 1 || undefined) +
              "=" +
              line.slice(i + 1);
            currentLine =
              currentLine.slice(0, +i + 1 || undefined) +
              "$" +
              line.slice(i + 2);
            this.map.tiles[j] = currentLine;
            nextLine =
              nextLine.slice(0, +(i - 1) + 1 || undefined) +
              "$" +
              nextLine.slice(i + 1);
            nextLine =
              nextLine.slice(0, +i + 1 || undefined) +
              "$" +
              nextLine.slice(i + 2);
